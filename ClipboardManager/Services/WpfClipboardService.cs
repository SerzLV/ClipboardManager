...
public sealed class WpfClipboardService : IClipboardService
{
        ...
        CancellationToken cancellationToken = default)
    {
        return ExecuteWithRetryAsync(
            () =>
            {
                ...
                return CreateTextSignature(text);
            },
            cancellationToken);
    }
    ...
    public async Task<bool> ClearIfCurrentSignatureMatchesAsync(
    ...
    public async Task<bool> ClearTextIfCurrentTextEqualsAsync(
        string text,
        CancellationToken cancellationToken = default)
    {
        var snapshot = await GetCurrentSnapshotAsync(cancellationToken);
        if (snapshot?.Kind != ClipboardContentKind.Text
    ...
    private static ClipboardContentSignature CreateTextSignature(string text)
    {
        var hash = SHA256.HashData(Encoding.UTF8.GetBytes(text));
        return new ClipboardContentSignature(ClipboardContentKind.Text, Convert.ToHexString(hash));
    }
